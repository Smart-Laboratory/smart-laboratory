...
//点击套餐按钮,已合并，暂时取消
	...
	$("#deleteAllTest").click(function(){
 	...
 	$("#batchAddResultsBtn").click(function(){
 		$.get("../audit/batchAddResults_statistic_getLoadValue",{lab:$("#labSelect").val()},function(data){
			var array = jQuery.parseJSON(data);
			for (var i=0 ; i < array.length ; i++) {
 	...
 	$("#batchAddResults_statisticBtn_get").click(function() {
 		...
 		jQuery("#batchAddResults_statistic_table").jqGrid({
 			url:"../audit/batchAddResults_statistic_get?bdate="+bdate+"&bsc="+bsc+"&bsb="+bsb+"&bse="+bse,
			datatype: "json",
