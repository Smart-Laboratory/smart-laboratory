...
public class SampleDaoHibernate extends GenericDaoHibernate<Sample, Long> implements SampleDao {
	...
	public List<Sample> getListBySampleNo(String sampleno) {
		return getSession().createQuery("from Sample where sampleNo='" + sampleno + "' order by upper(id)").list();
	}

	@SuppressWarnings("unchecked")
	public List<Sample> getNeedAudit(String day) {
		Query q =  getSession().createQuery("from Sample where sampleNo like '" + day + "%' and (auditStatus=0 or auditMark=4) order by upper(id)");
		q.setFirstResult(0);
		q.setMaxResults(100);
		return q.list();
	}
