                    ...
                    $.post(baseUrl + "/audit/add", {test: postStr, sample: sample, tcValues: tcValue}, function (data) {
                        ...
                        if (data) {
                            ...
                            if (data.size > 30 && $("#oneColumnShowBtn").prop("checked") == false) {
                                ...
                                jQuery("#rowed3").jqGrid("setGridParam", {
                                    url: "../audit/sample?id=" + ret.sample,
                                    editurl: "../audit/edit?sampleNo=" + ret.sample
                                }).trigger("reloadGrid");
                            }
...
function openOpStatusDialog() {
    layer.ready(function () {
        ...
        var description;
        var ids = "";
        $.get("../diagnosis/getDes", {
            diagnosis: $("#diagnosisValue").val(),
            sampleNo: $("#hiddenSampleNo").val()
        }, function (data) {
        ...
        if ($("#hisLastResult").val() == 1) {
            ...
            $.get("../print/historyChart", {
                sampleno: $("#hiddenSampleNo").val(),
                haslast: $("#hisLastResult").val()
            }, function (data) {
                $("#chartList").html("");
...
// }
    ...
    $("#packages").autocomplete({
        ...
        select: function (event, ui) {
            var result = true;
            $.get("../set/ylsf/ajax/profileTest", {
                test: ui.item.testIds,
                sample: $("#hiddenSampleNo").val()
            }, function (data) {
                var array = jQuery.parseJSON(data);
                for (var i = 0; i < array.length; i++) {
                    var result = true;
...
// 点击套餐按钮,已合并，暂时取消
    ...
    $("#batchAddResults_statisticBtn_save").click(function () {
        ...
        if (postStr != "") {
            $.post("../audit/batchAddResults_statistic_save", {
                postStr: postStr,
                bdate: bdate,
                bsc: bsc,
                bsb: bsb,
