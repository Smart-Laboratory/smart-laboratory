...
<script type="text/javascript">
	...
	function getList(patientId,blh) {
		...
		var mygrid = jQuery("#s3list").jqGrid({
        	url:"../manage/patientList/data?patientId="+ patientId + "&blh=" + blh, 
        	datatype: "json", 
        	width: 250, 
        	colNames:['ID', '<fmt:message key="audit.sample.number" />', '<fmt:message key="patientInfo.examinaim"/>', '<fmt:message key="patientInfo.resultstatus"/>'], 
        	colModel:[ 
        		{name:'id',index:'id', hidden:true},
	...
	$(function() {
		...
		$("#searchBtn").click(function() {
			...
			if (from != "" && to != "" && pName != "") {
				jQuery("#s3list").jqGrid("setGridParam",{
					url:"../manage/patientList/data?from="+from+"&to="+to+"&pName="+pName
				}).trigger("reloadGrid"); 
			} else if (doct != "") {
				jQuery("#s3list").jqGrid("setGridParam",{
					url:"../manage/patientList/data?doct="+doct
				}).trigger("reloadGrid"); 
			}
		...
		getList("<c:out value='${patientId}'/>","<c:out value='${blh}'/>");
	...
	<div style="position:absolute;right:0px; width: 220px;">
		<div id="historyTabs" style="display:none;">
			<ul>
				...
				<li><a href="#tabs-1"><fmt:message key="history.table"/></a></li>
			</ul>
